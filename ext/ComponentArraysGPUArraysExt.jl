...
const GPUComponentArray = ComponentArray{
    T, N, <:GPUArrays.AbstractGPUArray, Ax} where {T, N, Ax}
...
const GPUComponentVecorMat{
    T, Ax} = Union{GPUComponentVector{T, Ax}, GPUComponentMatrix{T, Ax}}
...
function Base.map(f, x::GPUComponentArray, args::Vararg{Union{
...
function Base.mapreduce(f, op, x::GPUComponentArray,
...
for (fname, op) in [(:sum, :(Base.add_sum)), (:prod, :(Base.mul_prod)),
    ...
    fname! = Symbol(fname, '!')
    @eval begin
        Base.$(fname!)(f::Function,
            r::GPUComponentArray,
            A::GPUComponentArray{T}) where {T} = GPUArrays.mapreducedim!(
